...
class HomeCubit extends Cubit<HomeState> {
  ...
  void _getOccasions() {
    _getOccasionInteractor.execute(null).then((occasion) {
      if (state is HomePrimaryState) {
        onChangeOccasionFilter(occasion);
      } else {
        emit(HomePrimaryState(occasion: occasion));
      }
    }).catchError((e) {
      // Do nothing for now
    });
  ...
  void _observeFoods() async {
    ...
    _foodStreamSubscription = stream.listen((foodList) {
      ...
      _emitFoodListWithFilter(occasion: state.occasion);
    });
  }
  ...
  void onSpinAnimEnd() {
    emit((state as HomePrimaryState)
        .copyWith(pickedFood: _pickedFood, foodResultTemp: _foodResultTemp));
  }
  ...
  void onChangeOccasionFilter(Occasion occasion) {
    if (occasion == state.occasion) return;
    _emitFoodListWithFilter(occasion: occasion);
  }
}
